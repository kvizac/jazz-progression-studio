...
export type Note = { midi:number; beat:number; duration:number; velocity:number; track:'chords'|'bass'; hand?:'left'|'right'; cellId:string; };
export const DEFAULT:Settings={...EXPRESSION,key:'F',mode:'major',style:'neosoul',form:'loop',bars:8,bpm:82,color:45,detail:'extended',groove:'pocket',swing:56,human:18,roll:12,bass:'soul',voicing:'open',sound:'electric',loop:true,click:false,countIn:false,seed:62719};
...
export function validateProject(x:unknown):Project{
 ...
 const percentages=['leftDensity','leftLevel','leftGate','tremoloDepth','density','syncopation','variation','gate','dynamics','arp','bassDensity','bassGate','bassVariation','bassApproach','bassOctaves','bassDynamics'] as const;
 if(!['off','auto','shells','root-fifth','broken'].includes(s.leftHand)||!Number.isFinite(s.tremoloRate)||s.tremoloRate<1||s.tremoloRate>8||percentages.some(k=>!Number.isFinite(s[k])||s[k]<0||s[k]>100)||[s.pocket,s.bassPocket].some(v=>!Number.isFinite(v)||v< -30||v>30)||![1,2,4].includes(s.arpRate)||![1,2].includes(s.arpOctaves)||!['up','down','pendulum','outside'].includes(s.arpPattern)||!['up','down','alternate'].includes(s.rollDirection))throw Error('Invalid expression settings.');
