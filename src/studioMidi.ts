...
export function midiBytes(p:Project,notes:Note[],range:[number,number]=[0,barCount(p.settings)],only:'all'|'chords'|'bass'='all'){
 ...
 for(const name of ['chords','bass'] as const){if(only!=='all'&&only!==name)continue;const ns=notes.filter(n=>n.track===name&&n.beat>=start&&n.beat<end);if(!ns.length)continue;const tr=m.addTrack();tr.name=name==='chords'?'Keys':'Bass';tr.channel=name==='chords'?0:1;tr.instrument.number=name==='bass'?33:s.sound==='electric'?4:s.sound==='guitar'?24:s.sound==='bass'?32:0;
