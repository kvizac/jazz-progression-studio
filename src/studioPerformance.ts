...
export function renderPerformance(p:Project,mode:'performance'|'blocks'='performance'):Note[]{
 ...
 for(let ci=0;ci<p.cells.length;ci++){
  ...
  const add=(midi:number,at:number,duration:number,velocity:number,track:Note['track'])=>{
   const beat=clamp(at,base,stop-.012);out.push({midi,beat,duration:clamp(duration,.005,stop-beat),velocity:clamp(velocity,.12,.97),track,cellId:c.id});
  };
  ...
  const pool=[...c.notes].sort((a,b)=>a-b);
  if(s.arpOctaves===2)pool.push(...c.notes.map(n=>n+12).filter(n=>n<=108));
  ...
  hits.forEach((_,hi)=>{
   ...
   const pitches=blocks||!arpeggiated? [...c.notes].sort((a,b)=>a-b):[arpPool[hi%arpPool.length]];
   ...
   pitches.forEach((midi,ni)=>{
    ...
    const velocity=blocks?.75:.69+(accent+(midi===Math.max(...c.notes)?.12:-.03)+(kr()-.5)*.15)*s.dynamics/100;
