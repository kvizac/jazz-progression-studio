...
export async function startPlayback(params:Params,chart:GeneratedChart,onBar:OnBar):Promise<PlaybackHandle>{
  ...
  transport.stop();transport.cancel(0);transport.position='0:0:0';transport.bpm.value=params.bpm;transport.timeSignature=4;
  transport.swing=params.groove==='swing'?Math.max(0,Math.min(1,(params.swing-.5)*4)):0;transport.swingSubdivision='8n';
  ...
  const harmonyStyle=params.style??params.preset;
  ...
  voicedBars.forEach((events,barIndex)=>{
    ...
    events.forEach(event=>{
      ...
      hits.forEach((beat,hitIndex)=>{
        ...
        },beatToTransportPosition(barIndex,beat));
      });
    });
  });
